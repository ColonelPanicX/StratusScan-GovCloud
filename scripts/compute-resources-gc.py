...
Title: AWS GovCloud Compute Resources All-in-One Export Script
Version: v1.0.0-GovCloud
Date: SEP-25-2025

Description:
This script performs a comprehensive export of all compute resources from AWS GovCloud
environments including EC2 instances, RDS databases, and EKS clusters. Each resource type
...
- EC2 instances with detailed configuration, security groups, and storage
- RDS instances with engine details, security, backup, and monitoring configuration
...
def print_title():
    ...
    print("====================================================================")
    print("AWS GOVCLOUD COMPUTE RESOURCES ALL-IN-ONE COLLECTION")
    print("====================================================================")
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if output_dir.exists():
                ...
                pattern = pattern_map.get(script_name, f'*{script_name.lower()}*.xlsx')
...
def create_compute_archive(output_files, account_name):
    """
    ...
    try:
        ...
        print(f"\n{'='*70}")
        print(f"CREATING COMPUTE RESOURCES ARCHIVE")
        print(f"{'='*70}")
...
def main():
    ...
    try:
        ...
        utils.log_info(f"Starting comprehensive compute resources collection from GovCloud...")
        ...
        # Define compute resource scripts to run
        scripts_dir = Path(__file__).parent
        compute_scripts = [
            {
                'name': 'EC2',
                'script': scripts_dir / 'ec2-export-gc.py',
                'description': 'EC2 instances and related resources'
            },
            {
                'name': 'RDS',
                'script': scripts_dir / 'rds-export-gc.py',
                'description': 'RDS database instances and clusters'
            },
            {
                'name': 'EKS',
                'script': scripts_dir / 'eks-export-gc.py',
                'description': 'EKS clusters, node groups, and add-ons'
            }
        ]
        ...
        print(f"\nPlanned exports:")
        for i, script_info in enumerate(compute_scripts, 1):
        ...
        for i, script_info in enumerate(compute_scripts, 1):
            ...
            utils.log_info(f"[{i}/{len(compute_scripts)}] Processing {script_info['name']}...")
        ...
        print(f"  Successful: {len(successful_exports)}/{len(compute_scripts)}")
        ...
        if output_files:
            ...
            archive_path = create_compute_archive(output_files, account_name)
            ...
            if archive_path:
                ...
                utils.log_govcloud_info(f"Compute resources archive created with GovCloud compliance markers")
                ...
                print(f"\nCompute Resources All-in-One Export Summary:")
